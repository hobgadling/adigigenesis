...
function read6502($address){
	...
	$j = $readaddr%SIZE;
	$retval = ($GLOBALS['grid'][$i][$j]['inst'] & 0xFF00) >> 8;
	if(mt_rand(0,200000) == 0){
		$bit_to_flip = mt_rand(0,7);
		$xor = 1 << $bit_to_flip;
		$retval = ($retval ^ $xor);
		echo "Read error \n";
	}
...
function write6502($address, $value){
	...
	$i = floor($readaddr/SIZE);
	$j = $readaddr%SIZE;
	$retval = ($value << 8) + ($GLOBALS['grid'][$i][$j]['inst'] & 0xFF00FF);
