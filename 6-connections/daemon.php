<?php
	...
	for($i = 0; $i < SIZE; $i++){
		for($j = 0; $j < SIZE; $j++){
			$GLOBALS['grid'][$i][$j] = array();
			$GLOBALS['grid'][$i][$j]['connections'] = 0;
			if(rand(0,5) == 0){
				$GLOBALS['grid'][$i][$j]['inst'] = 0;
			} else {
				$GLOBALS['grid'][$i][$j]['inst'] = mt_rand(0,0xFFFFFF);
			}
		}
	}
	...
	while(true){
		...
		for($i = 0; $i < SIZE; $i++){
			for($j = 0; $j < SIZE; $j++){
				if(mt_rand(0,100000000) == 0){
					$bit_to_flip = mt_rand(0,23);
					$xor = 1 << $bit_to_flip;
					$GLOBALS['grid'][$i][$j] = ($GLOBALS['grid'][$i][$j] ^ $xor);
					echo "Random flip\n";
				}
			}
		}
